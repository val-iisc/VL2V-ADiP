...
def todo_rename(records, selection_method, latex):
    ...
    for dataset in dataset_names:
        ...
        header_text = (
            f"Dataset: {dataset}, " f"model selection method: {selection_method.name}"
        )
        print_table(
            table, header_text, alg_names, list(col_labels), colwidth=20, latex=latex
        )
...
if __name__ == "__main__":
    ...
    records = records.filter(
        lambda r: r["dataset"] == args.dataset
        and r["algorithm"] == args.algorithm
        and r["test_env"] == args.test_env
    )
